...
public class MainFrame extends JFrame {
    ...
    public MainFrame(User user) {
        ...
        this.userManager = new UserManager("data/users.txt");
        ...
        // 设置窗口属性
        setTitle("学生信息管理系统 v2.0 - " + (currentUser.getRole() == User.Role.TEACHER ? "教师" : "学生") + "：" + currentUser.getUsername());
        setSize(1200, 700);
    ...
    private void createMenuBar() {
        ...
        // 菜单权限控制
        addItem.setEnabled(currentUser.getRole() == User.Role.TEACHER);
        editItem.setEnabled(currentUser.getRole() == User.Role.TEACHER);
        deleteItem.setEnabled(currentUser.getRole() == User.Role.TEACHER);
        ...
        importScoreItem.setEnabled(currentUser.getRole() == User.Role.TEACHER);
        manageCourseItem.setEnabled(currentUser.getRole() == User.Role.TEACHER);
        ...
        changePwdItem.addActionListener(e -> {
            ChangePasswordDialog dlg = new ChangePasswordDialog(this, userManager, currentUser.getUsername());
            dlg.setVisible(true);
        });
        ...
        if (currentUser.getRole() == User.Role.ADMIN) {
            ...
            JMenuItem registerItem = new JMenuItem("注册用户");
            registerItem.addActionListener(e -> {
                RegisterUserDialog dlg = new RegisterUserDialog(this, userManager);
                dlg.setVisible(true);
            });
            JMenuItem resetPwdItem = new JMenuItem("重置用户密码");
            resetPwdItem.addActionListener(e -> {
                AdminResetPasswordDialog dlg = new AdminResetPasswordDialog(this, userManager);
                dlg.setVisible(true);
            });
            adminMenu.add(registerItem);
            adminMenu.add(resetPwdItem);
    ...
    private void manageCourses() {
        if (currentUser.getRole() != User.Role.TEACHER) {
            JOptionPane.showMessageDialog(this, "仅教师可进行课程管理", "权限不足", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void addStudent() {
        if (currentUser.getRole() != User.Role.TEACHER) {
            JOptionPane.showMessageDialog(this, "仅教师可添加学生", "权限不足", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void editStudent() {
        if (currentUser.getRole() != User.Role.TEACHER) {
            JOptionPane.showMessageDialog(this, "仅教师可编辑学生", "权限不足", JOptionPane.WARNING_MESSAGE);
            return;
        }
        int selectedRow = studentTable.getSelectedRow();
        if (selectedRow == -1) {
    ...
    private void deleteStudent() {
        if (currentUser.getRole() != User.Role.TEACHER) {
            JOptionPane.showMessageDialog(this, "仅教师可删除学生", "权限不足", JOptionPane.WARNING_MESSAGE);
            return;
        }
        int selectedRow = studentTable.getSelectedRow();
        if (selectedRow == -1) {
    ...
    private void importScores() {
        if (currentUser.getRole() != User.Role.TEACHER) {
            JOptionPane.showMessageDialog(this, "仅教师可导入成绩", "权限不足", JOptionPane.WARNING_MESSAGE);
            return;
        }
